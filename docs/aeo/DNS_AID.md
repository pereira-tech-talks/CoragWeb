...
Publish HTTPS/SVCB records under the `_agents` namespace so AI agents can discover Corag before the first HTTP round-trip. Required for [isitagentready.com](https://isitagentready.com/) Discoverability → DNS-AID.
...
The scanner queries `_index._agents.<scanned-host>`. Scanning `https://corag.app` looks up `_index._agents.corag.app`. Scanning the apex looks up `_index._agents.corag.app`. Publish both while `v3` is the public preview hostname.

## Cloudflare DNS UI (manual)

In the **corag.app** zone → DNS → Records → Add record:

| Type | Name | Priority | Target | Value / SvcParams |
|------|------|----------|--------|-------------------|
| HTTPS | `_index._agents` | 1 | `corag.app` | `alpn="h2,h3" port=443` |
| HTTPS | `_mcp._agents` | 1 | `corag.app` | `alpn="h2,h3" port=443` |
| HTTPS | `_index._agents.v3` | 1 | `corag.app` | `alpn="h2,h3" port=443` |
| HTTPS | `_mcp._agents.v3` | 1 | `corag.app` | `alpn="h2,h3" port=443` |
