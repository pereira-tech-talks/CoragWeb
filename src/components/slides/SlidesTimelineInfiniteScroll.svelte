...
{:else}
  <div class="relative py-8">
        ...
        <div class={`ml-14 md:ml-0 md:w-1/2 ${isLeft ? 'md:pr-12' : 'md:pl-12 md:ml-auto'}`}>
          <article class="relative bg-corag-bg-elevated rounded-xl shadow-md hover:shadow-xl transition-all duration-300 overflow-hidden border border-corag-border hover:border-corag-primary/30 hover:-translate-y-1">
            ...
            <div class="p-5 pointer-events-none">
              ...
              {#if deck.eventName}
                <p class="text-sm text-corag-secondary mb-2">
                  {deck.eventName}
                  {#if deck.eventDate}
              ...
              <div class="flex flex-wrap items-center gap-2">
                ...
                {#if deck.provider}
                  <span class="text-xs px-2 py-0.5 rounded border border-corag-border text-corag-secondary">
                    {deck.provider}
                  </span>
                {/if}
              </div>
    ...
    {#if fetchError}
      <div role="alert" class="text-center py-8">
        ...
        <button
          on:click={loadMore}
          class="px-4 py-2 text-sm bg-corag-fill text-corag-on-fill rounded-lg hover:bg-corag-fill/90 transition-colors"
        >
  ...
  {#if allLoaded && renderedDecks.length > 0}
    <div class="flex flex-col items-center gap-4 pb-6">
      <div class="w-12 h-12 rounded-full bg-corag-primary shadow-lg shadow-corag-primary/25 flex items-center justify-center ring-4 ring-corag-primary/10 dark:ring-corag-primary/20">
      ...
      <div class="bg-corag-bg-elevated px-8 py-4 rounded-2xl shadow-md border border-corag-border text-center">
        <p class="text-sm font-semibold text-corag">
